...
<table class="table table-bordered"><thead><tr>
  ...
  let   firstCityStores = allStores.slice(0, 8);
    ...
    allStores.forEach(store => {
        mergedHTML += `<tr><td>${firstCityStores.includes(store) ? 'Hyderabad' : 'Hyderabad'}</td><td>.</td><td>${store}</td>`;
        // Projection columns
...
function getProjectedOrdersForAttempted(store) {
    // Use the Projected Orders | till 8-9 AM column (second column after store name)
    const projectionTable = document.getElementById("reportTable");
    if (!projectionTable) return 0;
    const rows = projectionTable.querySelectorAll("tbody tr");
    for (let row of rows) {
        const cells = row.querySelectorAll("td");
        if (cells[0] && cells[0].textContent.trim() === store) { // Corrected index to 0
            return parseInt(cells[2].textContent.trim()) || 0; // Corrected index to 2
        }
    }
    return 0;
}
...
function generateFinalTable() {
    ...
    finalStores.forEach(store => {
        ...
        // Get projected orders for this store (till 8-9 AM)
        const projectedOrders = getProjectedOrdersForAttempted(store);
