                ...
                return `
                    <tr>
                        ...
                        <td><input type="text" class="summary-input" id="input-${rowIdx}-2" data-row="${rowIdx}" data-col="2"></td>
                         <td><input type="text" class="summary-input" id="input-${rowIdx}-3" data-row="${rowIdx}" data-col="3"></td>
                    </tr>
                `;
            }).join("")}
        </tbody>
...
function enableSummaryArrowNavigation() {
    ...
    const colCount = 4; // Actual Riders, Idle rider, BF, Active Orders
...
document.getElementById("finalTableBtn").addEventListener("click", function () {
    ...
    summaryRows.forEach((row, idx) => {
       ...
       let summaryArr = Array.from(cells).slice(1, -4).map(td => td.textContent.trim());
const activeOrders = document.getElementById(`input-${idx}-3`)?.value || "";
summaryArr.push(actualRiders, idleRider, bf, activeOrders);
    });
...
<table class="table table-bordered"><thead><tr>
    ...
    allStores.forEach(store => {
        ...
        (summaryData[store] || summaryHeaderArr.map(() => "")).forEach((val, i) => {
            ...
            } else if (i >= summaryHeaderArr.length - 4) {
...
mergedHTML += `<td${hide ? ' class="hide-col"' : ''} style="color:${color};">${avg.toFixed(2)}%</td>`;
    ...
    mergedHTML += `<td id="merged-footer-userinput-${i - (summaryHeaderArr.length - 4)}">0</td>`;
    ...
    summaryHeaderArr.forEach((h, i) => {
        let hide = h.toLowerCase().includes('cancelled orders') || h.toLowerCase().includes('attempted orders') || h.toLowerCase().includes('deep pain') && !h.toLowerCase().includes('%');
       if (i === summaryHeaderArr.length - 4) {
    mergedHTML += `<td colspan="4" id="merged-footer-userinput-grand">0</td>`;
} else if (i < summaryHeaderArr.length - 4) {
    mergedHTML += `<td${hide ? ' class="hide-col"' : ''}></td>`;
}
    });
...
for (let i = 0; i < 4; ++i) {
    ...
    if (footerCell) {
        if (i === 2) { // BF column: show average as (sum of user input) / 8
            let bfInputs = Array.from(document.querySelectorAll('input[data-col="' + (summaryHeaderArr.length - 2) + '"]'));
            let bfSum = 0;
...
const colCount = 4; // Always 4 user input columns
...
function ActiveOrder() {
    // Collect all user input values for Actual Riders, Idle rider, BF, and Active Orders
    const actualRidersInputs = Array.from(document.querySelectorAll('.summary-input[data-col="0"]'));
    const idleRiderInputs = Array.from(document.querySelectorAll('.summary-input[data-col="1"]'));
    const bfInputs = Array.from(document.querySelectorAll('.summary-input[data-col="2"]'));
    const activeOrderInputs = Array.from(document.querySelectorAll('.summary-input[data-col="3"]'));

    // Helper to get mean value from input array
    function mean(inputs) {
        const values = inputs.map(input => parseFloat(input.value)).filter(val => !isNaN(val));
        if (values.length === 0) return 0;
        return (values.reduce((a, b) => a + b, 0) / values.length).toFixed(2);
    }

    // Calculate means
    const meanActualRiders = mean(actualRidersInputs);
    const meanIdleRider = mean(idleRiderInputs);
    const meanBF = mean(bfInputs);
    const meanActiveOrders = mean(activeOrderInputs);

    // Return all means as an object
    return {
        actualRiders: meanActualRiders,
        idleRider: meanIdleRider,
        bf: meanBF,
        activeOrders: meanActiveOrders
    };
}

const averages = ActiveOrder();
console.log(averages.actualRiders, averages.idleRider, averages.bf, averages.activeOrders);
